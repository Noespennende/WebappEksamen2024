...
import LessonCreateList from "./LessonCreateList";
...
const isValid = (items:any) => {
  ...
  export default function Create() {
    ...
    const { course } = useCourse(courseFields.slug);
    const lesson = useLesson(courseFields.slug, lessons[currentLesson]?.slug);
    const comments = useComment(courseFields.slug, lessons[currentLesson]?.slug);
    ...
    return (
      <>
        ...
        <form className="mt-8 max-w-4xl" data-testid="form" noValidate>
          {current === 0 ? (
            <CreateCourseFields courseFields={courseFields}
          ...
          {current === 1 ? (
            ...
            >
              <LessonCreateList lessons={lessons}
            currentLessonIndex={currentLesson}
            changeCurrentLesson={setCurrentLesson}
            addLesson={() => setLessons((prev) => [...prev, { title: "", slug: "", text: [] }])}
          />
              ...
              {lessons?.length > 0 ? (
                <CreateLessonFields lesson={lessons[currentLesson]}
          ...
          {current === 2 ? ( 
            <CourseReview courseFields={courseFields} lessons={lessons} />
